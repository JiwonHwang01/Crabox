<?php
  ...
  $code = $_GET["code"];;
  $state = $_GET["state"];;
  $redirectURI = urlencode("http://35.243.71.176/naver/navercallback.php"); //문자열을 url 쿼리로 사용할 수 있도록 encode 해주는 함수
 ...
 // echo "status_code:".$status_code."<br>";
  ...
  if($status_code == 200) {
    ...
    if($me_status_code == 200) { //서버에 user 정보 요청 성공
      ...
      $query = mysqli_query($link,"SELECT * FROM member2 WHERE id LIKE '".$userId."'");
      ...
      if($cnt == 0){ //새로운 회원이므로 memeber2 db에 회원 정보, SNS_INFO에 token 값 저장
        ...
        $sql1 = "UPDATE SNS_INFO SET type = 'NAVER' WHERE email ='".$email."'";
        $sql2 = "UPDATE SNS_INFO SET access_token = '".$access_token."' WHERE email ='".$email."'";
        $sql3 = "UPDATE SNS_INFO SET refresh_token = '".$refresh_token."' WHERE email ='".$email."'";
        mysqli_query($link,$sql1);
        mysqli_query($link,$sql2);
        mysqli_query($link,$sql3);
        mysqli_query($link,"INSERT INTO member2 SELECT (type, access_token) FROM SNS_INFO WHERE SNS_INFO.email = email");
